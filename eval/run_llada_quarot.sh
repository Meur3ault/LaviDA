...
accelerate launch --num_processes=8 \
    -m lmms_eval \
    --model llava_llada_quarot \
    --model_args pretrained=$1,conv_template=llada,model_name=llava_llada,a_bits=8,v_bits=8,k_bits=8,w_bits=8,w_clip=True,rotate=True,w_rtn=False,nsamples=128,cal_dataset=c4,seed=2 \
    --tasks $TASKS \
    --batch_size 1 \
    --gen_kwargs refix_lm=True \
    --log_samples \
    --log_samples_suffix llava_llada_quarot \
