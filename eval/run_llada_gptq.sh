...
accelerate launch --num_processes=8 \
    -m lmms_eval \
    --model llava_llada_gptq \
    --model_args pretrained=$1,conv_template=llada,model_name=llava_llada,wbits=4,group_size=128,desc_act=False,nsamples=128,calib_dataset=c4 \
    --tasks $TASKS \
    --batch_size 1 \
    --gen_kwargs refix_lm=True \
    --log_samples \
    --log_samples_suffix llava_llada_gptq \
